...
- [x] Long Stream `Read`/`Write`: a `byte[]` can't exceed `Int32.MaxValue`, so the chunking was unreachable *and* broke Read semantics. Validate args and delegate, preserving partial-read behaviour; overloads kept. Tests: partial read, bounds, negative.
