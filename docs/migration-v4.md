...
- **Void commands no longer take a status code and declare none.** `MapCommand<TRequest>` / `MapPutCommand<TRequest>` complete as the framework default (**200 OK, no body**). Opt into a specific code on the returned builder — `.Produces(StatusCodes.Status202Accepted)` — or return your own `IResult`. Semantic verbs keep their conventional codes: **Create maps still return 201**, **Delete (no response) still returns 204**.

**What to change:**
- Callers of the void `MapCommand<TRequest>(pattern, returnStatusCode)` (or `returnStatusCode, binding`) drop the status-code argument; if you relied on the previous `204` default, add `.Produces(StatusCodes.Status204NoContent)` and return `Results.NoContent()`, or use `MapDelete`.
